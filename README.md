...
Текущая стадия: **Iteration A / A.6 подготовлен, A.1-A.5 закоммичены, A.6 ожидает подтверждения и применения вручную**.
...
- A.5: GCP bootstrap scripts, deploy scripts и prereq checks.

Что ещё не применено в инфраструктуре:

- включение нужных GCP API, если ещё не выполнено через bootstrap;
- создание и проверка Artifact Registry;
...
- ручная верификация end-to-end smoke-тестов после деплоя.
...
## Порядок применения A.6

После коммита A.6 рекомендуемая последовательность такая:
...
9. Подключить remote connectors в Perplexity UI и выполнить первичный smoke-test.
10. Зафиксировать результаты smoke-test и, при необходимости, вернуться к A.2/A.3 для исправлений.
